...
* [ ] 12. ![[obsidian配置文件目录/Attachment/Pasted image 20231010143815.png]]
...
79. [ ] 构造函数在技术上是常规函数。不过有两个约定：
	...
	3. 当一个函数被使用 `new` 操作符执行时，它按照以下步骤：
		...
		3. 返回 `this` 的值。
		4. ![[obsidian配置文件目录/Attachment/Pasted image 20231019115008.png]]
		...
		7. new function() { … }![[obsidian配置文件目录/Attachment/Pasted image 20231019115319.png]]
		...
		9. 它可以被用在函数内部
			1. ![[obsidian配置文件目录/Attachment/Pasted image 20231019115514.png]]
		...
		12. 通常构造器没有 `return` 语句。这里我们主要为了完整性而提及返回对象的特殊行为。![[obsidian配置文件目录/Attachment/Pasted image 20231019115854.png]]
		13. 省略括号
			1.![[obsidian配置文件目录/Attachment/Pasted image 20231019115939.png]]
...
80. [ ]  可选链 "?."[手把手教你在webpack中，配置两种常用的polyfills配置，以ie11为例，配置babel-loader_babel兼容ie11-CSDN博客](https://blog.csdn.net/weixin_50794208/article/details/130830275)不要过度使用可选链
	我们应该只将 `?.` 使用在一些东西可以不存在的地方。例如，如果根据我们的代码逻辑，`user` 对象必须存在，但 `address` 是可选的，那么我们应该这样写 `user.address?.street`，而不是这样 `user?.address?.street`。那么，如果 `user` 恰巧为 undefined，我们会看到一个编程错误并修复它。否则，如果我们滥用 `?.`，会导致代码中的错误在不应该被消除的地方消除了，这会导致调试更加困难。![[obsidian配置文件目录/Attachment/Pasted image 20231019143725.png]]
	* *## [短路效应](https://zh.javascript.info/optional-chaining#duan-lu-xiao-ying) 
	* ![[obsidian配置文件目录/Attachment/Pasted image 20231019143839.png]]![[obsidian配置文件目录/Attachment/Pasted image 20231019143953.png]]
	...
	* 我们可以使用 `?.` 来安全地读取或删除，但不能写入![[obsidian配置文件目录/Attachment/Pasted image 20231019144207.png]]
...
81. [ ] symbol 类型
	...
	- symbol 不会被自动转换为字符串
	- ![[obsidian配置文件目录/Attachment/Pasted image 20231019144810.png]]
...
1. #原始类型的方法。 
	...
	15. 例如，我们有 `1.2345`，并且想把它舍入到小数点后两位，仅得到 `1.23`。![[obsidian配置文件目录/Attachment/Pasted image 20231019170513.png]]
...
86. 数组
	...
	2. [pop/push, shift/unshift 方法](https://zh.javascript.info/array#poppushshiftunshift-fang-fa)
		1. 栈：LIFO（Last-In-First-Out），即后进先出法则。![[obsidian配置文件目录/Attachment/Pasted image 20231020145232.png]]
		...
		13. ## [总结](https://zh.javascript.info/array#zong-jie)
			...
			![[obsidian配置文件目录/Attachment/Pasted image 20231020151326.png]]
...
85. [ ] 数组方法
	...
	![[obsidian配置文件目录/Attachment/Pasted image 20231023141829.png]]
		...
		- [arr.some(fn)](https://developer.mozilla.org/zh/docs/Web/JavaScript/Reference/Global_Objects/Array/some)/[arr.every(fn)](https://developer.mozilla.org/zh/docs/Web/JavaScript/Reference/Global_Objects/Array/every) 检查数组。
		    ...
		    我们可以使用 `every` 来比较数组：
		   ![[obsidian配置文件目录/Attachment/Pasted image 20231023142150.png]]
...
88. [ ] # 解构赋值
	...
	6. ![[obsidian配置文件目录/Attachment/Pasted image 20231023163651.png]]
89. [ ] ## [总结](https://zh.javascript.info/date#zong-jie)
	...
	有时我们需要更加精准的时间度量。JavaScript 自身并没有测量微秒的方法（百万分之一秒），但大多数运行环境会提供。例如：浏览器有 [performance.now()](https://developer.mozilla.org/zh/docs/Web/API/Performance/now) 方法来给出从页面加载开始的以毫秒为单位的微秒数（精确到毫秒的小数点后三位）：![[obsidian配置文件目录/Attachment/Pasted image 20231023170640.png]]
90. [ ] # JSON 方法，toJSON
	1. 重要的限制：不得有循环引用。
...
91. [ ] ![[obsidian配置文件目录/Attachment/Pasted image 20231024093632.png]]
...
93. [ ] 变量作用域，闭包
	1. 从视觉上看，`let i` 位于 `{...}` 之外。但是 `for` 构造在这里很特殊：在其中声明的变量被视为块的一部分。![[obsidian配置文件目录/Attachment/Pasted image 20231024105539.png]]
...
96. [ ] # "new Function" 语法
	...
	因此，此类函数无法访问外部（outer）变量，只能访问全局变量。![[obsidian配置文件目录/Attachment/Pasted image 20231024112345.png]]
...
98. [ ] # 装饰器模式和转发，call/apply![[obsidian配置文件目录/Attachment/Pasted image 20231024151746.png]]
	1. debounce防抖装饰器![[obsidian配置文件目录/Attachment/Pasted image 20231024153109.png]]
	2. throttle节流装饰器![[obsidian配置文件目录/Attachment/Pasted image 20231024153159.png]]
	...
	1. 延时装饰器![[obsidian配置文件目录/Attachment/Pasted image 20231024153302.png]]
	2. 间谍装饰器![[obsidian配置文件目录/Attachment/Pasted image 20231024153322.png]]
...
100. [ ] 箭头函数 
	1. ## [箭头函数没有 “this”](https://zh.javascript.info/arrow-functions#jian-tou-han-shu-mei-you-this)，不能对箭头函数进行 `new` 操作 ![[obsidian配置文件目录/Attachment/Pasted image 20231024160206.png]]
...
101. [ ] # 属性标志和属性描述符 第一种是 **数据属性**。![[obsidian配置文件目录/Attachment/Pasted image 20231024162136.png]]
102. [ ] # 属性的 getter 和 setter 它是 **访问器属性（accessor property）**![[obsidian配置文件目录/Attachment/Pasted image 20231024162152.png]]
103. 继承
	1. 原型继承
	...
	2. # F.prototype
		1. ![[obsidian配置文件目录/Attachment/Pasted image 20231024165721.png]]
		2. ![[Pasted image 20231024165803.png]]
		3. ![[obsidian配置文件目录/Attachment/Pasted image 20231024165856.png]]
		4. 在本章中，我们简要介绍了为通过构造函数创建的对象设置 `[[Prototype]]` 的方法。稍后我们将看到更多依赖于此的高级编程模式。	
			一切都很简单，只需要记住几条重点就可以清晰地掌握了：
			- **`F.prototype` 属性（不要把它与 `[[Prototype]]` 弄混了）在 `new F` 被调用时为新对象的 `[[Prototype]]` 赋值。**
			- `F.prototype` 的值要么是一个对象，要么就是 `null`：其他值都不起作用。
			- `"prototype"` 属性仅当设置在一个构造函数上，并通过 `new` 调用时，才具有这种特殊的影响。![[obsidian配置文件目录/Attachment/Pasted image 20231024170329.png]]
			- ![[obsidian配置文件目录/Attachment/Pasted image 20231024171243.png]]
			- ![[Pasted image 20231024171456.png]]
			- ![[obsidian配置文件目录/Attachment/Pasted image 20231024171507.png]]
	...
	3. # 原型方法，没有 __proto__ 的对象
		1. ![[obsidian配置文件目录/Attachment/Pasted image 20231024172025.png]]
		2. ![[obsidian配置文件目录/Attachment/Pasted image 20231024172744.png]]
101. class类 
	1. 类的方法之间没有逗号：对于新手开发人员来说，常见的陷阱是在类的方法之间放置逗号，这会导致语法错误。不要把这里的符号与对象字面量相混淆。在类中，不需要逗号。
	2. ![[obsidian配置文件目录/Attachment/Pasted image 20231024174709.png]]
	3. ![[obsidian配置文件目录/Attachment/Pasted image 20231024174657.png]]
	4. # 类继承
		...
		5. ## [总结](https://zh.javascript.info/class-inheritance#zong-jie)
			...
			补充：
			- 箭头函数没有自己的 `this` 或 `super`，所以它们能融入到就近的上下文中，像透明似的。
	5. # 静态属性和静态方法 :[从__proto__和prototype来深入理解JS对象和原型链 · Issue #9 · creeperyang/blog (github.com)](https://github.com/creeperyang/blog/issues/9)![[obsidian配置文件目录/Attachment/Pasted image 20231025110846.png]]
		...
		4. Function和Object的关系
			...
			![[obsidian配置文件目录/Attachment/Pasted image 20231025115525.png]]
	...
	7. # 类检查："instanceof"
		`instanceof` 操作符用于检查一个对象是否属于某个特定的 class。同时，它还考虑了继承。在许多情况下，可能都需要进行此类检查。例如，它可以被用来构建一个 **多态性（polymorphic）** 的函数，该函数根据参数的类型对参数进行不同的处理。![[obsidian配置文件目录/Attachment/Pasted image 20231025173212.png]]
		...
		如果我们想要获取内建对象的类型，并希望把该信息以字符串的形式返回，而不只是检查类型的话，我们可以用 `{}.toString.call` 替代 `instanceof`。![[obsidian配置文件目录/Attachment/Pasted image 20231025174039.png]]
